...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">لیست تولیدکننده‌ها</h3>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
            ...
            <tbody>
              <?php foreach ($producers as $producer): ?>
                <tr role="row" class="odd">
                  <td class="sorting_1">{{$producer->name}}</td>
                  <td>
                      <a href="{{route('producer.edit', ['producer'=>$producer->id])}}" type="button" class="btn btn-block btn-warning">ویرایش</a>
                  </td>
                  <td>


                    <form class="" action="{{route('producer.destroy', ['producer'=>$producer->id])}}" method="post">
                      {{csrf_field()}}
                      {{method_field('delete')}}
